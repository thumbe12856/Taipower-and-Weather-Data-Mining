...
for i in range(len(loadData)):
	...
	# when loading data until to another day.
	if tempDate != loadData[i]["create_date"]["$date"][:10]:

		# append tuple data (day of power data) to insertData.
		insertData.append(tuple((
			tempDate, northSupply, northUsage, centerSupply, centerUsage, southSupply, southUsage, eastSupply, eastUsage
		)))

		tempDate = loadData[i]["create_date"]["$date"][:10]
		northSupply = northUsage = centerSupply = centerUsage = southSupply = southUsage = eastSupply = eastUsage = 0.0

	# add temp data to variables.
...
for i in range(2, 24):
	...
	# append tuple data (day of power data) to insertData.
	insertData.append(tuple((
		date, northSupply, northUsage, centerSupply, centerUsage, southSupply, southUsage, eastSupply, eastUsage
	)))
	
# insert data to database.
insert = ("INSERT INTO `test` (date, northSupply, northUsage, centerSupply, centerUsage, southSupply, southUsage, eastSupply, eastUsage) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)")
